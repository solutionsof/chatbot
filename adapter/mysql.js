...
getData = (message = '', callback) => connection.query(
    `SELECT * FROM ${DATABASE_NAME}.bot_claves WHERE keywords LIKE '%${message}%'  LIMIT 1`,
    (error, results
        ) => {
...
getReply = (option_key = '', callback) => connection.query(
    `SELECT * FROM ${DATABASE_NAME}.bot_respuestas WHERE option_key = '${option_key}'  LIMIT 1`,
    (error, results
        ) => {
    const [response] = results;
...
getMessages = ( number ) => new Promise((resolve,reejct) => {
    try {
        connection.query(
        `SELECT * FROM ${DATABASE_NAME}.bot_respuestas WHERE number = '${number}'`, (error, results) => {
            if(error) {
                console.log(error)
            }
            const [response] = results;
